...
public class TerbinthBowItem extends BowItem {
    public TerbinthBowItem(Item.Settings settings) {
        super(settings);
    }
    @Override
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        ...
        if (!world.isClient) {
            ...
            persistentProjectileEntity.setDamage(25.0);
            if (f == 1.0f) {
                persistentProjectileEntity.setCritical(true);
            }
            if ((j = EnchantmentHelper.getLevel(Enchantments.POWER, stack)) > 0) {
                persistentProjectileEntity.setDamage(25 + (double)j * 0.5 + 0.5);
            }
